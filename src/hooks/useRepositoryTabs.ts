...
import { RepositoryTab, RepositoryInfo, BranchInfo, BranchHead, TagInfo, CommitInfo, FileStatus, ViewMode } from '../types/git';
...
export function useRepositoryTabs() {
  ...
  const loadRepositoryData = useCallback(async (tabId: string, persistedTabData?: PersistedTabData) => {
    try {
      const [branchesData, branchHeadsData, tagsData, remotesData, commitsData, statusData] = await Promise.all([
      ...
      setTabStates(prev => {
        ...
        newMap.set(tabId, {
          ...
          selectedCommitId: existingState?.selectedCommitId ?? persistedTabData?.selectedCommitId ?? null,
  ...
  const openRepository = useCallback(async (path: string): Promise<boolean> => {
    try {
